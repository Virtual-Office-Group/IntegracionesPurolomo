...
### Tipos de documentos para la recepcion

pueden diversos tipos de documentos a recepcionar, como por ejemplo:

| Nombre | Tipo de dato | Descripción |
| :--- | :--- | :--- |
| PurchaseOrder | String | Orden de compra generada en Business Central. |
| InboundTransfer | String | Transferencia entrante desde otro almacén. |

el tipo de documento se especifica en el campo `sourceDocument` dentro de cada línea de la recepción, lo que permite a Business Central identificar correctamente el origen de cada artículo recibido y aplicar las reglas de negocio correspondientes para su procesamiento.
...
body
{
  ...
  "warehouseReceiptLines": [
    { 
      "sourceDocument": "PurchaseOrder", // Tipo de documento de origen
...
### Plantillas y secciones de diario

Para la creacion de ajuste de inventario a partir de la API, se deben configurar y consultar la plantillas y secciónes de diario a utilizar para crear los asientos en el
 diario de productos.

Endpoint para consultar plantillas y secciones de diario:

```json
GET 
https://api.businesscentral.dynamics.com/v2.0/{{TenantID}}/{{enviromentName}}/api/virtualOfficeGroup/integration/v1.0/companies({{companyId}})/itemJournalTemplates?$expand=itemJournalSections
Content-Type: application/json
Authorization: Bearer {access_token}
```
...
```json
{
    "@odata.context": "https://api.businesscentral.dynamics.com/v2.0/30eb258d-51a4-4f58-9c88-bf72d4d3325a/TEST/api/virtualOfficeGroup/integration/v1.0/$metadata#companies(41cc1c74-5021-f011-9af7-000d3ac04986)/itemJournalTemplates",
    "value": [
        {
            "@odata.etag": "W/\"JzE5Ozk4NTI4NTExMDM3NjUyMjYwMDAxOzAwOyc=\"",
            "id": "e41ad6ab-1308-f111-8405-000d3ac0ae45",
            "name": "PRODUCTO",
            "description": "Diario Producto",
            "reasonCode": "",
            "systemModifiedAt": "2026-02-12T13:06:46.657Z",
            "itemJournalSections": [
                {
                  "@odata.etag": "W/\"JzIwOzE3MzY0ODAyMzIwNzUzNzI2NTA0MTswMDsn\"",
                  "id": "e51ad6ab-1308-f111-8405-000d3ac0ae45",
                  "name": "GENERICO",
                  "description": "Diario gen\u00e9rico",
                  "noSerie": "",
                  "reasonCode": "",
                  "systemModifiedAt": "2026-02-12T13:06:46.683Z"
                }
            ]
        }
    ]
...
**Campos de la respuesta (Plantilla)**

| Nombre | Tipo de dato | Descripción |
| :--- | :--- | :--- |
| @odata.context |  | La URL que define el esquema y el origen de los datos |
| @odata.etag | String | Un identificador de versión del registro. |
| id | Guid | El ID de la plantilla de diario. |
| name | Code[10] | nambre de la plantilla del Diario |
| description | Text[100] | descripción de la seccion de diario |
| reasonCode | Code[10] | Codigo de motivo asociado a la plantilla |
| systemModifiedAt | Text[20] | Fecha de la ultima modificacion |

**Campos de la respuesta (Secciones)**

| Nombre | Tipo de dato | Descripción |
| :--- | :--- | :--- |
| @odata.context |  | La URL que define el esquema y el origen de los datos |
| id | Guid | El ID de la seccion de diario. |
| code | Code[10] | nombre de la seccion del Diario |
| description | Text[100] | descripción de la seccion de diario |
| noSerie | Code[20] | serie numerica utilizada en la seccion | 
| reasonCode | Code[10] | Codigo de motivo asociado a la Sección |
| systemModifiedAt | Text[20] | Fecha de la ultima modificacion |
...
}
{
    "environment":{environment}, // Nombre del entorno
    "companyName":{companyID}, //codigo de la compañia 
    "journalTemplateName": "GENERAL", // codigo de plantilla de diario    
    "journalBatchName": "AJUSTE", // codigo de seccion de diario
